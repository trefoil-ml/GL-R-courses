...
subtitle: Adapted from chapter `3` of An Introduction to Statistical Learning
---
...
sizeAdvertising= c(nrow(Advertising),ncol(Advertising)) 
sizeAdvertising
```
...
we can also save our model in a text file   
...
Of course, we can also the `lm` function  to make the prediction:
```{r}
...
##  geom_smooth(mapping = aes(x = TV, y = sales))+ 
#  geom_smooth(method='lm',formula= sales~TV)
...
* Linear models rely upon a lot of assumptions (such as the features being independent), and if those
assumptions are violated (which they usually are), R-squared and p-values are less reliable.
...
How do we interpret the IsLarge coefficient? For a given amount of TV/Radio/Newspaper ad spending, being a small market is associated with an average increase in Sales of 256.56 widgets (as compared to a large market, which is called the baseline level).
...
Holding all other variables fixed, being a suburban area is associated with an average decrease in Sales of 246 widgets (as compared to the baseline level, which is rural). Being an urban area is associated with an average increase in Sales of 70 widgets (as compared to rural).
...
* TV:Radio: the effect of TV ads on sales increases by 1.09 for every $1000$ unit increase in Radio ads. For example at Radio ads of 0 the slope between TV ads and sales is almost equal $0.02$, at Radio ads of 10 the slope between TV ads and sales will be almost equal $0.0309 = 0.02+(10*1.09/1000)$   

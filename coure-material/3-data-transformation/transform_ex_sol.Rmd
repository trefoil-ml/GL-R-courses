...
1. How could you use `arrange()` to sort all missing values for coulmn dep_time to the start? (Hint: use `is.na()`).
...
```{r}
arrange(flights, desc(dep_delay), year , month,day)
```
...
```{r}
df <- select(flights, year, month, day, year, year)
df %>% distinct(year,month)
```
